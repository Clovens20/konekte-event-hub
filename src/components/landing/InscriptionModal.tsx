...
export const InscriptionModal = ({ isOpen, onClose }: InscriptionModalProps) => {
  ...
  // ✅ Écran de confirmation — redirige vers /acces-formation
  if (isConfirmed) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-foreground/50 backdrop-blur-sm animate-fade-in">
        <div className="bg-card rounded-3xl w-full max-w-md p-8 shadow-2xl text-center animate-scale-in">
          ...
          <p className="text-muted-foreground mb-6">
            Felisitasyon <strong>{formData.nomComplet}</strong>! Klike sou bouton an pou jwenn aksè nan fòmasyon an.
          </p>
          <button
            onClick={() => {
              handleClose();
              navigate('/acces-formation');
            }}
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-5">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setFormData({ ...formData, niveauExperience: e.target.value as 'Débutant' | 'Intermédiaire' | 'Avancé' | '' })}
            >
